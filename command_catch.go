...
func commandCatch(c *Config, cache *pokecache.Cache, u *User, loc ...string) error {
	url := fmt.Sprintf("https://pokeapi.co/api/v2/pokemon/%s", loc[0])
	...
	if pokemon.BaseExperience < 100 {
		pos = 2
	} else if pokemon.BaseExperience < 500 {
		pos = 5
	} else {
		pos = 8
	}
...
type Pokemon struct {
	...
	PastStats []interface{} `json:"past_stats"`
